...
本文研究\textbf{高维语义空间对齐}与对齐后的\textbf{语义漂移}表征（源码\url{https://github.com/Shelly-icecream/semantic-space-alignment-concept-drift}）。数据层面，所用向量为分别基于
...
综合正交残差、锚点余弦与验证子集上的距离变化可见，本次语义空间对齐是有效的。
一方面，$\|Q^{\mathsf{T}}Q-I_d\|_\infty$ 处于 $10^{-6}$ 量级，说明所得映射在数值上基本满足正交约束，
能够在不明显破坏向量长度与夹角结构的前提下完成坐标旋转。另一方面，保留锚点的平均余弦达到
$0.6634$，且验证子集在对齐后表现出平均距离下降、平均余弦上升的趋势，说明微博侧词向量经 $Q$
映射后与人民日报侧词向量具有更强的几何可比性。

同时，对齐效果并非在所有词类上均匀出现。高频实词的改善幅度较小，而功能词的改善更明显，
这表明正交映射主要校正了两套词向量空间之间由训练和基选择带来的整体坐标差异；
对于受话题、语体和语境分布影响较大的实词，仍会保留真实的域间语义差别。
因此，本文认为该对齐结果已经足以支撑后续的跨域比较，但后续语义漂移解释仍需结合词级、
群体级与几何级指标共同判断，不能将对齐后的残差简单等同于噪声。
...
词级结果以连续分布及分位数概括整体形态与右尾厚度，$P_{95}$ 以上约 $5\%$ 的词作为后续个案讨论的重点候选。

图~\ref{fig:word-shift-dist} 为上述子集的 $\mathrm{Shift}(w)$ 直方图。主体质量集中在约 $0.37$--$0.54$（$P_{25}$--$P_{75}$）一带，
 表明多数高频非锚点词在对齐后方向差异处于中等水平；$P_{90}$ 以上形成偏厚的右尾（至 $P_{99}\approx0.71$），极端最大值约 $0.88$ 的个案宜结合其在排序中的位
 置与语料语境审慎判读。

图~\ref{fig:word-shift-auto} 给出按 $\mathrm{Shift}(w)$ 降序排列的前 $20$ 个词：
其取值多位于 $P_{99}$ 以上。
考察集已限定为 Top-$5000$ 高频非锚点词，榜单词条在两侧语料中均具一定共现规模，
高 $\mathrm{Shift}$ 反映的是对齐后\textbf{方向差异偏大}的局部信号。
该列表适合作为\textbf{候选筛查}清单；具体词条宜结合其在频序中的位置、分位数与语料用法核查。

图~\ref{fig:word-shift-dist} 左尾上 $\mathrm{Shift}(w)$ 极低的情形亦不宜直接读作“语义最稳定”：
两域中均相对少见的词，向量方向估计方差较大，所得极低 $\mathrm{Shift}$ 可能来自估计噪声而非稳定的跨域一致；
对该类词条的解读宜与右尾极端值一样，结合词频与语料证据审慎对待。

在当前对齐设定下，词级漂移可概括为三点：其一，$\mathrm{Shift}(w)$ 在 
Top-$5000$ 非锚点词上呈“主体集中、右尾偏厚”，多数词方向差异居中而高值词构成稀疏尾部；
其二，自动 Top 列表提供按漂移量\textbf{可排序}的局部候选，便于后续个案复核；
其三，分布两端均受估计噪声影响，解读时宜结合词频、分位数与语境证据。
\subsubsection{现实解释}
从实验结果看，多数词漂移量处于中部区间，说明空间对齐后两域共有词整体仍具有可比性；
右尾高漂移词则需要进一步区分估计不稳定与真实语料差异。因此，本文将词级余弦漂移理解为\textbf{跨语料用法差异的候选信号}，而不是单词语义变化的直接结论。
...
解读时宜将 $\overline{J}_k$ 视为子集上的汇总水平，具体词条可结合 $J_k(w)$ 排序、$\mathrm{Shift}(w)$ 与语料证据作个案复核。

\subsubsection{现实解释}
邻域 Jaccard 处于中低水平，说明两域差异不仅出现在向量方向上，也出现在近邻词集合的重新组合上。
该指标与 $\mathrm{Shift}$ 一样，提供可排序、可复核的\textbf{局部候选信号}，用于定位值得进一步结合语料用法核查的词条。
...
后续结构差异并非由一侧空间整体塌缩或膨胀造成。也就是说，两类语料仍共享相近的总体语义距离量级，
但在局部方向展开、邻域重组和词群组织方式上保留差异。
...
正交对齐主要校正的是两域之间的整体旋转或反射差异，即尽量在保持几何距离关系不变的前提下对齐两个空间；但当前相对残差平方
仍约为 $0.7697$，说明在最优正交映射之后，交集词之间仍保留了较大比例的几何不一致。

这表明两类语料的语义差异并不只是全局坐标系不同，还可能包含局部拉伸、压缩、邻域重组等无法由正交变换解释的结构变化。
换言之，人民日报与微博中的相同词汇虽然可以被放入统一空间进行比较，但它们在不同语料环境中的共现关系和语义组织方式
...
与“真实语义变化”不可等同；自动 Top 列表中的专名噪声无法从估计上根除，解读时宜结合词频与语境。
...
方向移动与邻域重排；结合排除锚点后的 Top-$5000$ $\mathrm{Shift}$ 分布统计与专名噪声讨论，词级结果宜解释为\textbf{局部、可复核}的跨语境
...
\subsubsection{现实意义与应用价值}
总体而言，本文工作的现实意义在于：它提供了一套从\textbf{不可直接比较的高维词向量}
